...
bool VlRandomizer::nextFlat(VlRNG& rngr, const std::vector<std::string>& uniqueExprs) {
    ...
    if (!hasRandc && !m_checkOnly && !hasFrozenVar() && Unigen2(rngr, uniqueExprs)) return true;
    ...
    // If randc vars have used values, this may be cycle exhaustion - retry without exclusions
    if (!sat && hasRandc && !m_randcUsedValues.empty()) {
        m_randcUsedValues.clear();
        os << "(pop 1)\n";
        os << "(check-sat)\n";
...
bool VlRandomizer::Unigen2(VlRNG& rngr, const std::vector<std::string>& uniqueExprs) {
    ...
    // Drain cached solutions from previous UG2 run if available.
    if (!m_ug2.loTreshWitnesses.empty()) {
        Witness witness = std::move(m_ug2.loTreshWitnesses.back());
        m_ug2.loTreshWitnesses.pop_back();
        if (VL_UNCOVERABLE(!writeBackWitness(witness))) return false;
        return true;
    }
    ...
    if (!m_ug2.paramsValid) {
        ...
        // caches parameters
        if (!estimateParameters(solver, rngr)) {
            solver << "(reset)\n";
            return false;
        }
    ...
    if (VL_UNCOVERABLE(generateSamples(solver, rngr))) {  // Always true in practice
        Witness witness = std::move(m_ug2.loTreshWitnesses.back());
        m_ug2.loTreshWitnesses.pop_back();
        const bool written = writeBackWitness(witness);
        solver << "(reset)\n";
        return written;
    }
    solver << "(reset)\n";  // LCOV_EXCL_LINE
...
// parseSMTNum only rejects a missing '#' or an unknown base, so this tells us
// upfront whether set() would take the value.
static bool isParsableSMTNum(const std::string& val) {
    const size_t pos = val.find('#');
    if (VL_UNCOVERABLE(pos == std::string::npos || pos + 1 >= val.size())) return false;
    const char base = val[pos + 1];
    return base == 'b' || base == 'o' || base == 'h' || base == 'x';  // LCOV_EXCL_LINE
}

bool VlRandomizer::writeBackWitness(const Witness& witness) {
    // A failed randomize() must leave the variables alone (IEEE 18.6.3), so check
    // every value before writing any of them.
    for (const auto& varEntry : witness) {
        for (const auto& elemEntry : varEntry.second) {
            if (VL_UNCOVERABLE(!isParsableSMTNum(elemEntry.second))) {
                // LCOV_EXCL_START
                VL_WARN_MT(__FILE__, __LINE__, "randomize",
                           "Internal: Solver returned a value that could not be parsed");
                return false;
                // LCOV_EXCL_STOP
            }
        }
    ...
    for (const auto& varEntry : witness) {
        ...
        if (VL_UNCOVERABLE(isFrozenVar(varEntry.first, *varp))) continue;  // Sanity check
        for (const auto& elemEntry : varEntry.second) {
            if (varp->dimension() > 0) {
...
static std::vector<std::pair<std::string, int>> s_bsatOrder;
// Ask for a solution, block it, ask again - so every answer is a new one.
int VlRandomizer::bsat(std::iostream& solver, size_t bound, std::vector<Witness>& witnesses,
                       VlRNG* diversifyRngp) {
    witnesses.clear();
    ...
    std::vector<std::pair<std::string, int>>& order = s_bsatOrder;
    order.clear();
    for (const auto& var : m_vars) {
    ...
    while (witnesses.size() < bound) {
        ...
        if (diversifyRngp && !witnesses.empty()) {
            ...
            ++assumeNum;
            readNonBlankLine(solver, result);
        }
        if (result != "sat") {
            solver << "(check-sat)\n";
            readNonBlankLine(solver, result);
        ...
        if (result != "sat") {
            ...
            if (VL_UNLIKELY(result != "unsat")) return -1;
            break;
        }
        ...
        const auto expectChar = [&solver](char want) {
            char got = '\0';
            solver >> got;
            return got == want;
        };
        ...
        Witness witness;
        bool badReply = !expectChar('(');  // opening '(' of the whole get-value response
        for (const auto& entry : order) {
            if (VL_UNLIKELY(badReply)) break;
            // opening '(' of this (expr value) pair
            if (VL_UNCOVERABLE(!expectChar('('))) {
                badReply = true;  // LCOV_EXCL_LINE
                break;  //  LCOV_EXCL_LINE
            }
            std::string exprHead;
            solver >> exprHead;  // bare var name, or literally "(select" if an array element
            if (exprHead == "(select") {
                readUntilBalanced(solver);  // discard rest of expr
                // reading positionally needs the requested order back
            } else if (VL_UNCOVERABLE(exprHead != entry.first)) {
                badReply = true;  // LCOV_EXCL_LINE
                break;  // LCOV_EXCL_LINE
            }
            std::string value;
            std::getline(solver, value, ')');
            if (VL_UNCOVERABLE(!solver || value.empty())) {
                badReply = true;  // LCOV_EXCL_LINE
                break;  // LCOV_EXCL_LINE
            }
            witness[entry.first][entry.second] = value;
        }  // LCOV_EXCL_BR_LINE
        // closing ')' of the whole get-value response
        if (VL_UNCOVERABLE(badReply || !expectChar(')'))) {
            VL_WARN_MT(__FILE__, __LINE__, "randomize",
                       "Internal: Unexpected solver response to (get-value ...)");
            return -1;
        }
...
// --- UniGen2 constants ---
static constexpr double ug2Kappa = 0.638;  // Parameter controling the tolerance of the uniformity
                                           // guarantee. Set at the paper's default value.
// Values used in the large space mode:
static constexpr int ug2HiTreshLargeSpace = 16;  // Num of solutions to enumerate
...
bool VlRandomizer::estimateParameters(std::iostream& solver, VlRNG& rngr) {
    // Below formulas are taken from the UniGen2 paper, Sec. 4, Algorithm 1
    const double pivot = ceil(4.03 * pow((1 + 1 / ug2Kappa), 2));
    m_ug2.hiTresh = static_cast<int>(ceil(1 + sqrt(2) * (1 + ug2Kappa) * pivot));
    m_ug2.loTresh = static_cast<int>(floor(pivot / (sqrt(2) * (1 + ug2Kappa))));

    // A space so small can never reach loTresh, so enumerate it whole instead.
    ...
    if (tinyCellSize >= 1 && tinyCellSize <= 60) {
        m_ug2.hashBits = 0;
        m_ug2.loTresh = tinyCellSize;  // batch = the whole enumerated set
        m_ug2.hiTresh = tinyCellSize + 1;
        return true;
    }
    ...
    m_ug2.hiTresh = ug2HiTreshLargeSpace;
    return true;
}

bool VlRandomizer::generateSamples(std::iostream& solver, VlRNG& rngr) {
    ...
    for (int t = 0; t < 3; ++t) {  // LCOV_EXCL_BR_LINE - Always finds a candidate in practice
        ...
        unigenXors(solver, rngr, i);
        const int solCount
            = bsat(solver, m_ug2.hiTresh, witnesses, m_ug2.isLargeSpace ? &rngr : nullptr);
        if (m_ug2.loTresh <= solCount && (solCount < m_ug2.hiTresh || m_ug2.isLargeSpace)) {
            m_ug2.lastSuccessI = i;
            const int take
                = m_ug2.isLargeSpace ? std::min(ug2BatchLargeSpace, solCount) : m_ug2.loTresh;
            ...
            for (int k = 0; k < take; ++k) {
                ...
                m_ug2.loTreshWitnesses.push_back(std::move(witnesses[indices[k]]));
            }
            solver << "(pop 1)\n";
            return true;
        }
...
bool VlRandomizer::nextPhased(VlRNG& rngr, const std::vector<std::string>& uniqueExprs) {
    ...
    if (layers.empty()) return nextFlat(rngr, uniqueExprs);
