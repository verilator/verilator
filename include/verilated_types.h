...
private:
    ...
    mutable std::vector<std::pair<T_Key, T_Value>> m_modify_input;

    Map& map() const {
        if (!m_modify_input.empty()) {
            for (const auto& input : m_modify_input)
                m_map.emplace(input.first, input.second);
            m_modify_input.clear();
        }
        return m_map;
    }
...
public:
    ...
    int size() const { return m_map.size() + m_modify_input.size(); }
    // Clear array. Verilog: function void delete([input index])
    void clear() {
        m_map.clear();
        m_modify_input.clear();
    ...
    int exists(const T_Key& index) const { const auto& map_ = map(); return map_.find(index) != map_.end(); }
    ...
    T_Value& at(const T_Key& index) {
        ...
        if (it == m_map.end()) {
            if (m_modify_input.empty() ||
                (!m_modify_input.empty() && m_modify_input.back().first < index)) {
                m_modify_input.emplace_back(index, m_defaultValue);
                return m_modify_input.back().second;
            }
            const auto pair = std::make_pair(index, m_defaultValue);
            auto it2 = std::lower_bound(m_modify_input.begin(), m_modify_input.end(), pair,
			    [](const auto &value, const auto &input) {
                                   return value.first < input.first;
			    });
            if (index < it2->first)
                it2 = m_modify_input.insert(it2, std::make_pair(index, m_defaultValue));
            return it2->second;
        }
        return it->second;
    }
    ...
    // For save/restore
    const_iterator begin() const { const auto &map_ = map(); return map_.begin(); }
    const_iterator end() const { const auto &map_ = map(); return map_.end(); }
    ...
    template <typename Func> VlQueue<T_Value> find_first(Func with_func) const {
        const auto& map_ = map();
        const auto it
            = std::find_if(map_.begin(), map_.end(), [=](const std::pair<T_Key, T_Value>& i) {
    ...
    template <typename Func> VlQueue<T_Value> find_last(Func with_func) const {
        const auto& map_ = map();
        const auto it
            = std::find_if(map_.rbegin(), map_.rend(), [=](const std::pair<T_Key, T_Value>& i) {
                  return with_func(i.first, i.second);
              });
