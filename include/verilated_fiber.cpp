...
thread_local VlFiber* VlFiber::t_currentFiberp = nullptr;
...
void VlFiber::resume() {
    ...
    VlFiber* const previousFiberp = t_currentFiberp;
    t_currentFiberp = this;  // We are now the current fiber
    ...
    t_currentFiberp = previousFiberp;
    if (m_done) resumeWaiter();
}

void VlFiber::yield() {
    VlFiber* const currentFiberp = t_currentFiberp;
    if (!currentFiberp) return;  // Not in fiber, nothing to yield

    // Save fiber's state and return to caller
    t_currentFiberp = nullptr;
    currentFiberp->m_ctx.yield();

    // Returns here when fiber is resumed
    t_currentFiberp = currentFiberp;  // Restore current fiber
}
