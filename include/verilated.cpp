...
WDataOutP VL_ZVALUE_RESET_W(int obits, WDataOutP outwp) VL_MT_SAFE {
    // Not inlined to speed up compilation of slowpath code
    return VL_ZVALUE_W(obits, outwp);
}
...
double VL_ITOR_D_W(int lbits, const WDataInP lwp) VL_PURE {
    ...
    if (ms_word == 1) return static_cast<double>(VL_SET_QW_T(lwp));
        ...
        if (VL_GT_W_TT(w, integer, max64Bit)) {
            ...
            *ptr = '\0';
            while (VL_GT_W_TT(w, v, zero)) {
                ...
                *ptr = "0123456789"[VL_SET_QW_T(mod)];
                VlWide<w> divided;
        ...
        if (!inPct && pos[0] == '%') {
            ...
            if (formatAttr == VL_VFORMATATTR_COMPLEX) {  // printed as string
                ...
                if (lbits <= VL_QUADSIZE) {
                    ...
                    ld = VL_SET_QW_T(lwp);  // Low 64 bits, for %c/%t
                }
                if (fmt == 'p') {
