...
AstNodeModule* V3LinkDotIfaceCapture::findCorrectClone(AstNodeModule* wrongOwnerp,
    ...
    UINFO(9, "finalizeIfaceCapture disambiguate: wrong "
                 << wrongOwnerp->name() << " parent=" << wrongParentp->name()
...
int V3LinkDotIfaceCapture::fixDeadRefs(AstRefDType* refp, AstNodeModule* containingModp,
    ...
    if (refp->typedefp()) {
        ...
        if (typedefModp && typedefModp->dead()) {
            ...
            if (containingModp) {
                cloneModp = findCloneViaHierarchy(containingModp, typedefModp);
            }
            if (cloneModp) {
    ...
    if (refp->refDTypep()) {
        ...
        if (targetModp && targetModp->dead()) {
            AstNodeModule* cloneModp = nullptr;
            if (containingModp) {
                cloneModp = findCloneViaHierarchy(containingModp, targetModp);
            }
...
AstNodeModule* V3LinkDotIfaceCapture::disambiguateTarget(AstNodeModule* curOwnerp,
                                                        AstNodeModule* ownerModp,
                                                        AstNodeModule* correctModp,
                                                        AstRefDType* refp,
                                                        const CapturedEntry& entry,
                                                        const ReachableInfo& reachable,
                                                        const char* label) {
    ...
    forEachImpl(
        [ownerModp, &ownerName](const CapturedEntry& e) {
            ...
            UINFO(9, "iface capture forEachOwned filter: ref="
                         << (e.refp ? e.refp->name() : "<null>") << " cellPath='"
                         << e.cellPath << "' ownerMod="
                         << (e.ownerModp ? e.ownerModp->name() : "<null>")
...
void V3LinkDotIfaceCapture::captureTypedefContext(
    ...
    UINFO(9, indentFn() << "iface capture capture request stage=" << stageLabel << " typedef="
                        << refp << " name=" << refp->name() << " dotPos=" << dotPos
...
int V3LinkDotIfaceCapture::fixDeadRefsInTypeTable() {
    ...
    for (AstNode* nodep = v3Global.rootp()->typeTablep()->typesp(); nodep;
        ...
        nodep->foreach([&](AstRefDType* refp) {
            ...
            fixed += fixDeadRefs(refp, containingModp, "type table");
        });
    ...
    for (AstNode* nodep = v3Global.rootp()->modulesp(); nodep; nodep = nodep->nextp()) {
        if (AstNodeModule* const modp = VN_CAST(nodep, NodeModule)) {
            ...
            modp->foreach([&](AstRefDType* refp) {
                fixed += fixDeadRefs(refp, modp, modName.c_str());
            });
        }
    }
    return fixed;
    ...
    forEach([&](const CapturedEntry& entry) {
        ...
        UINFO(
            ...
            "finalizeIfaceCapture Phase3 entry: refp="
                ...
                << ")" << " storedOwnerMod="
                << (entry.ownerModp ? entry.ownerModp->name() : "<null>") << " cellPath='"
                << entry.cellPath << "' cloneCellPath='" << entry.cloneCellPath << "' typedefp="
                << (refp->typedefp() ? refp->typedefp()->name() : "<null>") << " refDTypep="
                << (refp->refDTypep() ? refp->refDTypep()->name() : "<null>")
        ...
        if (refp->typedefp()) {
            ...
            if (AstNodeModule* const fixModp
                = disambiguateTarget(tdOwnerp, ownerModp, correctModp, refp, entry, reachable,
                                     "typedefp")) {
                const string& tdName = refp->typedefp()->name();
        ...
        if (refp->refDTypep()) {
            AstNodeModule* const rdOwnerp = findOwnerModule(refp->refDTypep());
            if (AstNodeModule* const fixModp
                = disambiguateTarget(rdOwnerp, ownerModp, correctModp, refp, entry, reachable,
                                     "refDTypep")) {
