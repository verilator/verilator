...
class AwaitVisitor final : public VNVisitor {
    ...
    struct CFuncCache final {
        ...
        enum State : uint8_t {
            ...
            VISITING,  // Visiting - members are used as a scratchpad so, they must not change
    ...
    void visit(AstCFunc* nodep) override {
        ...
        CFuncCache& value = m_cfuncsCache(nodep)[static_cast<size_t>(m_gatherVars)];
        {
            ...
            case CFuncCache::UNINITIALIZED: {
                // Save current state
                VL_RESTORER_SWAP(m_processDomains);
                VL_RESTORER_SWAP(m_writtenBySuspendable);
            ...
            case CFuncCache::VISITING: return;  // Break recursion
