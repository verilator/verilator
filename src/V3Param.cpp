...
class ParamSubstVisitor final {
    ...
    AstNode* paramValuep(AstVar* varp) {
        ...
        if (sourcep) {
            const VNTreeUP<AstVar> holderp = AstVar::newDTypeHolder(
                varp->fileline(), "__Vpinparam", varp->subDTypep()->cloneTree(false));
            holderp->valuep(sourcep->cloneTree(false));
            substitute(holderp.get());
            if (!holderp->exists([](const AstVarRef*) { return true; })) {
                V3Const::constifyParamsNoWarnEdit(holderp.get());
