...
class ParamProcessor final {
    ...
    void resolveDefaultParams(AstNode* nodep) {
        ...
        for (size_t i = 0; i < m_classParams.size(); ++i) {
            ...
            if (AstRefDType* const refDtp = VN_CAST(reqDtp->subDTypep(), RefDType)) {
                if (AstParamTypeDType* const sourceParamp
                    ...
                    for (size_t j = 0; j < m_classParams.size(); ++j) {
                        if (m_classParams[j].first == sourceParamp) {
                            m_classParams[i].second = static_cast<int>(j);
                            break;
                        }
                    }
        ...
        int numPins = 0;
        std::vector<AstPin*> pinsByIndex(m_classParams.size(), nullptr);
        {
            size_t idx = 0;
            for (AstPin* pinp = paramsp; pinp && idx < m_classParams.size();
                 pinp = VN_AS(pinp->nextp(), Pin)) {
                pinsByIndex[idx++] = pinp;
                numPins++;
            }
        }

        // For each missing parameter, get its pin from dependency or direct default
        for (size_t paramIdx = numPins; paramIdx < m_classParams.size(); paramIdx++) {
            ...
            if (sourceParamIdx >= 0) {
                AstPin* const sourcePin = pinsByIndex[sourceParamIdx];
                if (sourcePin) newPin = sourcePin->cloneTree(false);
            }
