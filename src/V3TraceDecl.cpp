...
class TraceDeclVisitor final : public VNVisitor {
    ...
    void newDeclFunc(AstNodeDType* nodep) {
        ...
        if (pair.second) {
            string dtypeName = nodep->cDTypeName();
            UINFO(1, "NEW TYPE FUNC: " << nodep->prettyDTypeName(false));  // NOCOMMIT
            ...
            pair.first->second = newCFunc(flp, m_dtypeNames.get(name));
            pair.first->second->user2p(m_traVscp);
        }

        AstCCall* const callp = new AstCCall{flp, pair.first->second};
        callp->dtypeSetVoid();
        callp->argTypes(callArgs);
        AstTraceDecl* const declp = addTraceDecl(VNumRange{}, nodep->width(), callp);
        ...
        if (pair.second) {
            ...
            if (AstStructDType* dtypep = VN_CAST(nodep, StructDType)) {
                declStruct(dtypep, true);
            } else if (AstUnpackArrayDType* dtypep = VN_CAST(nodep, UnpackArrayDType)) {
                declUnpackedArray(dtypep, true);
            } else if (AstPackArrayDType* dtypep = VN_CAST(nodep, PackArrayDType)) {
                declPackedArray(dtypep, true);
            } else {
                UASSERT_OBJ(false, nodep, "Creating a trace function for an unexpected type");
            }
