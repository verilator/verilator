...
class WidthVisitor final : public VNVisitor {
    ...
    void visit(AstNodeUOrStructDType* nodep) override {
        ...
        bool isSoftPackedUnion = VN_IS(nodep, UnionDType) && VN_CAST(nodep, UnionDType)->isSoft();
        ...
        if (VN_IS(nodep, UnionDType) || nodep->packed()) {
            ...
            for (; itemp; itemp = VN_CAST(itemp->backp(), MemberDType)) {
                ...
                if (VN_IS(nodep, UnionDType)) {
                    int itemWidth = itemp->width();
                    if (!first && !isSoftPackedUnion && itemWidth != width) {
                        itemp->v3error("Hard packed union members must have equal size (IEEE "
                                       "1800-2023 7.3.1)");
                    }
