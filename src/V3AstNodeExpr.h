...
// === AstNodeUniop ===
class AstABits final : public AstNodeUniop {
    // Node to extract bits representing aval from a four-state value
public:
    AstABits(FileLine* const flp, AstNodeExpr* const lhsp)
        : ASTGEN_SUPER_ABits(flp, lhsp) {
        dtypeSetBitUnsized(lhsp->width(), lhsp->widthMin(), lhsp->dtypep()->numeric(), false);
    }
    ASTGEN_MEMBERS_AstABits;
    string emitVerilog() override { V3ERROR_NA_RETURN(""); }
    string emitC() override { V3ERROR_NA_RETURN(""); }
    string emitSimpleOperator() override { V3ERROR_NA_RETURN(""); }
    bool cleanOut() const override { return true; }
    void numberOperate(V3Number& out, const V3Number& lhs) override { out.opExtractABits(lhs); }
    bool cleanLhs() const override { return true; }
    bool sizeMattersLhs() const override { return false; }
    const char* broken() const override {
        BROKEN_RTN(!v3Global.opt.fourstate());
        BROKEN_RTN(!(lhsp() && lhsp()->dtypep()->isShuffledFourstate()));
        BROKEN_RTN(VN_IS(lhsp(), Const));
        return nullptr;
    }
};
...
class AstBBits final : public AstNodeUniop {
    // Node to extract bits representing bval from a four-state value
public:
    AstBBits(FileLine* const flp, AstNodeExpr* const lhsp)
        : ASTGEN_SUPER_BBits(flp, lhsp) {
        dtypeSetBitUnsized(lhsp->width(), lhsp->widthMin(), lhsp->dtypep()->numeric(), false);
    }
    ASTGEN_MEMBERS_AstBBits;
    string emitVerilog() override { V3ERROR_NA_RETURN(""); }
    string emitC() override { V3ERROR_NA_RETURN(""); }
    string emitSimpleOperator() override { V3ERROR_NA_RETURN(""); }
    bool cleanOut() const override { return true; }
    void numberOperate(V3Number& out, const V3Number& lhs) override { out.opExtractBBits(lhs); }
    bool cleanLhs() const override { return true; }
    bool sizeMattersLhs() const override { return false; }
    const char* broken() const override {
        BROKEN_RTN(!v3Global.opt.fourstate());
    ...
    AstCCast(FileLine* fl, AstNodeExpr* lhsp, int setwidth, int minwidth = -1)
        ...
        if (setwidth) {
            ...
            dtypeSetBitUnsized(setwidth, minwidth, VSigning::UNSIGNED,
                               lhsp->dtypep()->isShuffledFourstate());
        }
    }
    // cppcheck-suppress constParameterPointer
    // cppcheck-suppress constParameterCallback
