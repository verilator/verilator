...
class TraceVisitor final : public VNVisitor {
    ...
    std::unordered_map<AstNodeDType*, DtypeFuncs>
        m_dtypeNonConstFuncs;  // Full / Chg funcs per type
    std::unordered_map<AstNodeDType*, AstCFunc*> m_dtypeConstFuncs;  // Const func per type
    ...
    AstCFunc* newCFunc(VTraceType traceType, AstCFunc* topFuncp, uint32_t funcNum,
        ...
        if (isTopFunc) {
            ...
            if (declp) {
                if (traceType == VTraceType::CHANGE && v3Global.opt.useTraceOffload()) {
                    // NOCOMMIT -- reduce copypasta
                    funcp->addStmtsp(new AstCStmt{flp,  //
                                                  "const uint32_t base VL_ATTR_UNUSED = "
                                                  "vlSymsp->__Vm_baseCode + offset - 1;\n"});
                    funcp->addStmtsp(
    ...
    AstCFunc* createConstDtypeTraceFunctions(const AstTraceDecl* declp) {
        ...
        if (pair.second) {
            ...
            AstCFunc* funcp = newCFunc(VTraceType::CONSTANT, nullptr, 0, 0, declp);

            for (AstNode* stmtp = declp->dtypeCallp()->funcp()->stmtsp(); stmtp;
                 stmtp = stmtp->nextp()) {
                if (AstTraceDecl* fieldDeclp = VN_CAST(stmtp, TraceDecl)) {
                    AstTraceInc* incp = new AstTraceInc{flp, fieldDeclp, VTraceType::CONSTANT};
    ...
    void createConstTraceFunctions(const TraceVec& traces) {
        ...
        for (auto it = traces.cbegin(); it != traces.end(); ++it) {
            ...
            if (actSet.count(TraceActivityVertex::ACTIVITY_NEVER)) {
                ...
                FileLine* const flp = declp->fileline();
                if (declp->dtypeCallp()) {
                    AstCFunc* funcp = createConstDtypeTraceFunctions(declp);
