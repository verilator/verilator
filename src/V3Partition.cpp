...
public:
    ...
    static const MTaskEdge* toEdge(GraphWay way, const EdgeHeap::Node* nodep) {
        // Offset of the node within the MTaskEdge
        const size_t offset
            = reinterpret_cast<size_t>(&(reinterpret_cast<MTaskEdge*>(0)->m_edgeHeapNode[way]));
        return reinterpret_cast<const MTaskEdge*>(reinterpret_cast<const char*>(nodep) - offset);
    }
...
uint32_t LogicMTask::critPathCostWithout(GraphWay way, const V3GraphEdge* withoutp) const {
    ...
    if (!maxp) return 0;
    if (MTaskEdge::toEdge(inv, maxp) != withoutp) return maxp->key().m_score;
...
void LogicMTask::dumpCpFilePrefixed(const V3Graph* graphp, const string& nameComment) {
    ...
    for (LogicMTask* nextp = startp; nextp;) {
        ...
        if (EdgeHeap::Node* const maxp = nextp->m_edgeHeap[GraphWay::FORWARD].max()) {
            nextp = MTaskEdge::toEdge(GraphWay::FORWARD, maxp)->toMTaskp();
        } else {
            nextp = nullptr;
        }
    }
