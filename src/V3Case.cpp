...
private:
            ...
            if (nodep->uniquePragma() || nodep->priorityPragma()) {
                ...
                for (uint32_t i = 0; i < numCases; ++i) {  // iterate through all case items
                    for (AstEnumItem* itemp = enumDtp->itemsp(); itemp;
                         itemp
                         = VN_AS(itemp->nextp(), EnumItem)) {  // iterate through all enum values
                        if (m_valueItem[i]) {
                            AstConst* const constp = VN_AS(itemp->valuep(), Const);
                            AstConst* const itemvp = VN_AS(m_valueItem[i]->op1p(), Const);
                            if (itemvp->num().toDecimalU().compare(constp->num().toDecimalU())
                                == 0) {  // match
                                foundMatch = true;
                            } else {  // no match
                                UINFO(9, "Value " << std::hex << i << " " << m_valueItem[i]->op1p()
                                                  << " " << itemvp->num().toDecimalU() << " => "
                                                  << itemp->prettyName() << " numeric value "
                                                  << constp->num().toDecimalU() << endl);
                            }
                        }
                    }
                    if (m_valueItem[i]) {
                        if (!foundMatch) {
                            suppressCaseIncWarn = false;  // warning should not be suppressed since
                                                          // not all values were supplied
                        } else {
                            foundMatch = false;
                        }
                    }
    ...
    bool isCaseTreeFast(AstCase* nodep) {
        ...
        if (!caseIsEnumComplete(nodep, numCases)) { return false; }
