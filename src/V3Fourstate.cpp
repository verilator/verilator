...
//
// - Creates shuffled signals at the end of verilation.
//   Components of every wide (>64 bits) four-state signal are merged into one:
//       VlWide<N> a_value;
//       VlWide<N> a_xz;
//     are turned into:
//       VlWide<2 * N> a;
//
//   Important note here is that the count of words in shuffled signal
//   IS NOT equal to: ceil((width * 2) / 32), instead it is equal to:
//   ceil(width / 32) * 2
//
//   Data in the shuffled signal is stored as follows:
//     [aval0, bval0, aval1, bval1, ...]
//     aval<i> - ith word from the a_value
//     bval<i> - ith word from the a_xz
//
//*************************************************************************
// TODOs:
...
class FourstateLogicTypePropagator final : public VNVisitor {
    ...
    void visit(AstCond* const nodep) override {
        iterateChildrenSeparately(nodep);
        // nodep->condp() is omitted since it does not change type of a whole expression
    ...
    void visit(AstCMethodHard* const nodep) override {
        iterateChildrenSeparately(nodep);
        // The `false` part is not entirely correct but for cases where it should be true (getting
        // array element) we do not support arrays itself so, it may stay this way for now
        setFourstate(nodep, false, m_fourstateInSubtree);
    }
...
class FourstateVisitor final : public VNVisitor {
    ...
    class FourstateExpressionValueVisitor final : public FourstateExpressionVisitor {
        ...
        void visit(AstCReset* const cresetp) override {  // LCOV_EXCL_LINE
            cresetp->v3fatalSrc("This shall never be reached - CReset shall never be under "
                                "anything but AstNodeAssign");
        }
        ...
        void visit(AstNodeVarRef* const varRefp) override {
            ...
            newp->dtypeSetBitSized(varRefp->varp()->width(), varRefp->varp()->dtypep()->numeric());
            m_resultp = newp;
        }
    ...
    AstNodeExpr* getFourstateExpressionValue(AstNodeExpr* const exprp, bool putIntoTmp = false) {
        if (AstCReset* const cresetp = VN_CAST(exprp, CReset)) {
            ...
            return cresetp->cloneTree(false);
        }
    ...
    void visit(AstNodeAssign* const nodep) override {
        ...
        if (isFourstate(nodep->lhsp())) {
            ...
            if (VL_UNLIKELY(!lhsVarRefp)) {
                nodep->v3warn(E_UNSUPPORTED, "Unsupported: Fourstate LHS other than a simple "
                                             "variable reference with --fourstate");
                return;
            }
            AstNodeAssign* const assignXZp = nodep->cloneTree(false);
            {
                pushDeletep(assignXZp->lhsp()->unlinkFrBack());
                pushDeletep(assignXZp->rhsp()->unlinkFrBack());
            ...
            if (AstAssignW* const assignWValuep = VN_CAST(nodep, AssignW)) {
                ...
                if (const AstNode* const timingControlp = assignWValuep->timingControlp()) {
                    timingControlp->v3warn(
                        E_UNSUPPORTED,
                        "Unsupported: Continuous assignment delays with --fourstate");
                }
            }
    ...
    void visit(AstCase* const nodep) override {
        ...
        if (isFourstate(nodep->exprp())) {
            nodep->v3warn(E_UNSUPPORTED,
                          "Unsupported: Case statements with four-state value as an "
                          "expression with --fourstate");
        } else {
            iterate(nodep->exprp());
        }
    ...
    void visit(AstCaseItem* const nodep) override {
        for (AstNodeExpr* condp = nodep->condsp(); condp;
            ...
            if (isFourstate(condp)) {
                nodep->v3warn(E_UNSUPPORTED,
                              "Unsupported: Four-state case items values with --fourstate");
            } else {
                iterate(condp);
            }
        }
        ...
        if (!varp->isFourstateConstruct()) {
            if (AstNodeExpr* const exprp = VN_CAST(nodep->exprp(), NodeExpr)) {
                if (VL_UNLIKELY(!(VN_IS(exprp, NodeVarRef) || VN_IS(exprp, Const)))) {
                    ...
                    exprp->v3warn(
                        E_UNSUPPORTED,
                        "Unsupported: Cells with pins that are not a variable reference or a "
                        "constant with --fourstate");
                    return;
                }
                if (needsSplitting(varp->dtypep())) {
                    AstPin* const newp = new AstPin{
                        nodep->fileline(), nodep->pinNum(),
                        nodep->name().empty() ? "" : nodep->name() + FOURSTATE_XZ_SUFFIX,
                    ...
                    case VDirection::INPUT: {
                        pushDeletep(exprp->unlinkFrBack());
                        nodep->exprp(getTwoStateCast(exprp));
                    } break;
                    case VDirection::OUTPUT: {
                        if (AstNodeVarRef* const varrefp = VN_CAST(exprp, NodeVarRef)) {
                            pushDeletep(varrefp->unlinkFrBack());
                            FileLine* const flp = varrefp->fileline();
                            m_modp->addStmtsp(
                                new AstAlways{flp, VAlwaysKwd::ALWAYS, nullptr,
                                              new AstAssign{flp, getFourstateExpressionXZ(varrefp),
                                                            createZeroOrOnesp(varrefp)}});
                            nodep->exprp(getFourstateExpressionValue(varrefp));
                        } else {
                            exprp->v3warn(E_UNSUPPORTED, "Unsupported: Four-state non variable "
                                                         "reference output pin with --fourstate");
                        }
                    } break;
                    default: {
                        exprp->v3warn(E_UNSUPPORTED, "Unsupported: Ports with direction other "
    ...
    void visit(AstModportVarRef* const nodep) override {
        if ((nodep->exprp() && isFourstate(nodep->exprp()))
            ...
            nodep->v3warn(E_UNSUPPORTED, "Unsupported: Modports with --fourstate");
        } else {
            iterateChildren(nodep);
        }
    }
...
class FourstateShuffleVisitor final : public VNVisitor {
    ...
    VDouble0 m_shuffledVars;  // Count of shuffled variables

    static bool needsShuffle(const AstVar* const varp) {
        return varp->isWide() && (varp->fourstateComplementp() || varp->isFourstateComplement());
    }

    static AstNodeExpr* sameABTree(const AstNodeExpr* const aexprp,
                                   const AstNodeExpr* const bexprp) {
        if (const AstABits* const abitsp = VN_CAST(aexprp, ABits)) {
            if (const AstBBits* const bbitsp = VN_CAST(bexprp, BBits)) {
                AstNodeExpr* const exprp = bbitsp->lhsp();
                if (abitsp->lhsp()->sameTree(exprp)) return exprp;
            }
        }
        return nullptr;
    }
    AstVar* getCreateShuffledVariantp(AstVar* varp) {
        ...
        const size_t pos = varp->name().rfind(FOURSTATE_XZ_SUFFIX);
        UASSERT_OBJ(pos != std::string::npos, varp,
                    "Four-state complementary value (xz part) shall have '"
                        << FOURSTATE_XZ_SUFFIX << "', but it is named: " << varp->name());
        ...
        resultp->name(resultp->name().erase(pos, sizeof(FOURSTATE_XZ_SUFFIX)));
        ...
        for (AstVar* varp = nodep->funcp()->argsp(); varp; varp = VN_AS(varp->nextp(), Var)) {
            ...
            if (needsShuffle(varp)) {
                ...
                if (AstVar* const complement = varp->fourstateComplementp()) {
                    ...
                    pushDeletep(exprp->nextp()->unlinkFrBack());
    ...
    void visit(AstNodeVarRef* const nodep) override {
        ...
        nodep->unlinkFrBack(&relinker);
        if (isComplement) {
            relinker.relink(new AstBBits{flp, nodep});
        } else {
            relinker.relink(new AstABits{flp, nodep});
        }
        nodep->varp(varp);
    }
    void visit(AstFourstateExpr* const nodep) override {
        iterateChildren(nodep);
        if (AstNodeExpr* const exprp = sameABTree(nodep->valuep(), nodep->xzp())) {
    ...
    void visit(AstNodeAssign* const nodep) override {
        iterateChildren(nodep);
        if (AstNodeAssign* const prevp = VN_CAST(nodep->prevp(), NodeAssign)) {
            if (AstNodeExpr* const lhsp = sameABTree(nodep->lhsp(), prevp->lhsp())) {
                if (AstNodeExpr* const rhsp = sameABTree(nodep->rhsp(), prevp->rhsp())) {
                    pushDeletep(prevp->unlinkFrBack());
                    pushDeletep(nodep->lhsp()->unlinkFrBack());
                    pushDeletep(nodep->rhsp()->unlinkFrBack());
                    nodep->lhsp(lhsp);
                    nodep->rhsp(rhsp);
                } else if (VN_IS(prevp->rhsp(), Const) && VN_IS(nodep->rhsp(), Const)) {
                    AstConst* const prevConstp = VN_AS(prevp->rhsp(), Const)->unlinkFrBack();
                    prevConstp->num().setXZFromXZComplement(VN_AS(nodep->rhsp(), Const)->num());
                    pushDeletep(prevp->unlinkFrBack());
                    pushDeletep(nodep->lhsp()->unlinkFrBack());
                    pushDeletep(nodep->rhsp()->unlinkFrBack());
                    prevConstp->dtypep(nodep->findBitDType(prevConstp->width(),
                                                           prevConstp->widthMin(),
                                                           prevConstp->dtypep()->numeric(), true));
                    nodep->lhsp(lhsp);
                    nodep->rhsp(prevConstp);
                }
            }
        }
    }
