...
This note explains the FSM-focused regression tests under ``test_regress/t/``
and how they are grouped. It is intended as a current inventory of the FSM
regression surface on this branch, not a historical consolidation log.

Core Detection
--------------

- ``t_cover_fsm_basic``: canonical one-block FSM with reset arcs and annotated
  coverage output.

  - proves the baseline single-register one-block ``if (rst) case (state)``
    extraction path

- ``t_cover_fsm_two_proc_multi``: grouped supported two-process and three-block
  styles, plus nearby supported core variants and nearby unsupported shapes
  that should stay uninferred.
...
- ``t_cover_fsm_beginif``: supported ``begin/end`` wrapping inside transition
  branches.

  - proves that branch-local block structure does not prevent one-block arc
    extraction
...
- ``t_cover_fsm_reset_multi``: grouped warning-oriented reset-policy cases.
  ...
  - multiple reset writes to the same state variable warn and suppress
    reset-arc modeling
...
- ``t_cover_fsm_plain_always_warn_multi_bad``: grouped warning-only near-FSM
  plain edge-sensitive ``always`` cases.

  - selector is ``state_q``
  - selector is ``state_d``
  - ``default`` is only supported because ``fsm_arc_include_cond`` is enabled

- ``t_cover_fsm_plain_always_ignore_multi``: grouped plain-``always`` shapes
  the warning scan must ignore silently.
...
- ``t_cover_fsm_nextstate_overwrite_warn``: next-state default is overwritten
  ...
  - canonical ``state_d = state_q`` default is present, but a later overwrite
    before ``case (state_d)`` makes the pattern unsupported
...
- ``t_cover_fsm_case_next_ok_multi``: grouped accepted canonical
  ...
  - accepted canonical ``case(state_d)`` with an unrelated pre-case bit-select
    assignment
...
- ``t_cover_fsm_enumwide_bad``: enum-backed state too wide for supported FSM
  ...
  - enum typing alone is not enough if the state width exceeds the supported
    range
...
- ``t_cover_fsm_combo_same_warn_bad``: same-module combo candidates that
  should warn.

  - multiple combinational candidates for the same state register must produce
    a warning, not duplicate FSMs

- ``t_fsmmulti_combo_multi_warn_bad``: grouped multi-candidate combo-warning
  cases.

  - related combinational multi-candidate warning shapes share one
    warning-oriented surface

- ``t_fsmmulti_same_bad``: duplicate same-state candidates.

  - repeated candidates for the same logical FSM should not produce duplicate
    modeled machines
...
- ``t_fsmmulti_warn_off``: multi-candidate warnings disabled.
  ...
  - warning suppression controls should disable FSMMULTI noise without changing
...
- ``t_cover_fsm_decldump``: declaration-dump side outputs for FSM extraction.
  - verifies the declaration dump still names and emits the expected FSM
    coverage declarations

Metacomments and Options
------------------------

- ``t_fsm_metacmt_dump``: metacomment dump coverage for FSM-related source
  annotations.

  - verifies FSM metacomments are preserved and dumped in the expected form

- ``t_opt_table_fsm``: option-table coverage for ``--coverage-fsm``.

  - verifies the command-line option is exposed consistently in the option
    table machinery
...
- ``t_vlcov_fsm_report``: report-generation coverage checks for FSM coverage output formatting and content.
  - verifies ``verilator_coverage`` report presentation for FSM state and arc
    data

Notes
-----

- Tests ending in ``_ok`` keep the FSM and usually check annotated output.
