...
// Port inlining makes a true alias, unlike sub/sub2's comb retargeting.
module subpass (
...
module t #(
  ...
  logic [6:0] pass_out;
  subpass u_subpass (.port_in(keep), .port_out(pass_out));

  // Explicit public_flat_rw vs lazy reconstruction
  logic [6:0] pinned_rw /* verilator public_flat_rw */;
  assign pinned_rw = keep ^ 7'h5;
  logic [6:0] lazy_ro;
  assign lazy_ro = keep + 7'h1;
  ...
  // Shapes that bail group reconstruction
  ...
  // Read only in the block that writes it: hits the completeness floor
  logic [6:0] floor_only;
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      observe <= result ^ floor_only ^ alias2 ^ sub_out ^ pass_out ^ pinned_rw ^ lazy_ro
