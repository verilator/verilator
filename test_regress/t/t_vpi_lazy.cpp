...
int main(int argc, char** argv) {
    ...
    // The deposit marker and the lazy epoch are separate stamps: the write above landed in real
    // storage and so bumped the epoch, which must not retire the reconstructed deposits.
    checkInt("t.alias1 (deposit survives an epoch bump)", alias1, 0x15);
    checkInt("t.alias2 (deposit survives an epoch bump)", alias2, 0x33);

    // A row already carrying a deposit skips the refresh, so a partial second put merges with
    // the deposit and not with a value recomputed from keep. Octal stores three bits at a
    // time, leaving alias1[6:3] alone: 0x15 -> 0x17 merging with the deposit, 0x27 had the row
    // been refreshed from keep (0x20) first.
    // COPY-PATH COVERAGE, NOT CONE-ROW COVERAGE. t.alias1 and t.alias2 are VLVF_LAZY_COPY rows
    // aliasing t.keep, so their deposit is the descriptor's own stamp in datapRefresh, not a
    // __Vlazydep word consulted by a generated cone body. Nothing here exercises the per-row
    // deposit guard; t_vpi_lazy_deposit is what covers that.
    s_vpi_value octWr{};
    octWr.format = vpiOctStrVal;
    char octVal[] = "7";
    octWr.value.str = octVal;
    if (!vpi_put_value(alias1, &octWr, nullptr, vpiNoDelay)) {
        std::printf("%%Error: failed to re-write through alias t.alias1\n");
        ++errors;
    }
    checkInt("t.alias1 (partial second deposit)", alias1, 0x17);
    checkInt("t.alias2 (unchanged by second alias1 put)", alias2, 0x33);
