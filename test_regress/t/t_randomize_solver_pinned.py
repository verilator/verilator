...
test.compile(verilator_flags2=['+define+T_PINNED'])

# Fully pinned value: every diversity assumption conflicts and is dropped
test.execute()
test.file_grep(test.run_log_filename, r'NPASS=5')

# Error instead of the unsat-assumptions reply: base solution stands
logfile = test.obj_dir + "/vlt_sim_err.log"
test.execute(logfile=logfile,
             run_env='VERILATOR_SOLVER=' + test.t_dir +
             '/randomize_solver_tamper.py TAMPER=err_reply TAMPER_AT=2')
