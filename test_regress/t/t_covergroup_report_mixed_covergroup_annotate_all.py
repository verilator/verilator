...
mixed_annot = test.obj_dir + "/annotated_mixed_all"
pure_annot = test.obj_dir + "/annotated_pure_all"
...
if "type=line" in text or "type=branch" in text:
    raise RuntimeError(f"Structural coverage leaked into covergroup annotate-all output:\n{text}")
if "type=covergroup" not in text:
    raise RuntimeError(f"Covergroup annotate-all output missing functional points:\n{text}")
...
for expected in (
    ...
    if expected not in text:
        raise RuntimeError(f"Missing expected annotate-all covergroup point:\n{expected}\n---\n{text}")
