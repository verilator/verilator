...
module t;
  ...
  initial begin
    for (int cycle = 0; cycle < 6; ++cycle) begin
      ...
      if (cycle == 2) begin
        force single_chain.a[0] = 1'b1;
        force narrow_chain.a[0] = 7'h65;
        force wide_chain.a[0] = 65'h0_fedc_ba98_7654_3210;
      end
      if (cycle == 4) begin
        release single_chain.a[0];
        release narrow_chain.a[0];
        release wide_chain.a[0];
      end
      #1;
      foreach (single_copy[i]) begin
        `checkh(single_copy[i], (cycle >= 2 && cycle < 4) ? 1'b1 : source_value[0]);
        `checkh(narrow_copy[i], (cycle >= 2 && cycle < 4) ? 7'h65 : source_value[6:0]);
        `checkh(wide_copy[i], (cycle >= 2 && cycle < 4) ? 65'h0_fedc_ba98_7654_3210 : source_value);
      end
    end
    $write("*-* All Finished *-*\n");
    $finish;
