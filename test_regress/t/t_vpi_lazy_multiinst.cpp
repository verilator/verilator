...
int main(int argc, char** argv) {
    ...
    // multiinst
    {
        ...
        // Each alias of a helper target holds its own shadow, so a deposit through one is NOT
        // visible through its siblings - which is what --public-flat-rw does, the aliases being
        // distinct nets. It leaves the other instance alone, and is recomputed on the next eval.
        const int ctr = readInt(ctrh);
        s_vpi_value wr{};
        wr.format = vpiIntVal;
        ...
        checkInt("t.u0.din_copy (after put)", u0Copy, ctr & 0x3c);
