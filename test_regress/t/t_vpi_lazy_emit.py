...
test.file_grep(test.stats, r'VPI, lazy reconstructed\s+(\d+)', 33)
test.file_grep(test.stats, r'VPI, lazy copy descriptors\s+(\d+)', 16)

# __Vlazyepoch is sized to the module's group count; copy aliases take no slot.
test.file_grep(root, r'VlUnpacked<QData/\*63:0\*/,\s*13>\s*__Vlazyepoch;')
...
# alias_dtype: a sign-mismatched alias folds like any other - no storage of its own, and its row
# reports its own VLVF_SIGNED rather than the canonical's.
test.file_grep_not(root, r't__DOT__a_diff;')
test.file_grep_not(root, r't__DOT__a_same;')
test.file_grep(syms, r'\{"a_diff", offsetof\(\S+ \S*__Vlazyrecon__\d+_\d+\)[^\n]*VLVF_SIGNED')

# Each alias's row reports its own sign/bitvar flags, taken from its own shadow's dtype
