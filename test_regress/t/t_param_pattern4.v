...
module t;
  ...
  // Pattern that carries its own data type, so the pattern's type is used as-is and the
  // template var's shared data type is left untouched (its size must not leak to siblings
  // such as u_wide/u_narrow above, which override the same parameter to other sizes)
  dut #(
      .ARRAY_LEN(3),
      .ARRAY_PARAM(arr3_t'{1, 2, 3})
  ) u_typedpat (
      .o_sum(o_typedpat)
  );

  // A typed pattern whose size differs from other instances of the same module, exercised
  // together with the bare-pattern instances so any cross-instance dtype sharing shows up
  dut #(
      .ARRAY_LEN(4),
      .ARRAY_PARAM(arr4_t'{1, 2, 3, 4})
  ) u_typedpat_wide (
      .o_sum(o_typedpat_wide)
  );
