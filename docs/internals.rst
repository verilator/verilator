...
Publicly writable variables
~~~~~~~~~~~~~~~~~~~~~~~~~~~

DFG synthesis must preserve observable writes to public storage, not only
the values computed by a process. This applies to variables exposed with
:vlopt:`--public-flat-rw` or the ``public_flat_rw`` metacomment. An
intermediate value can be independent of a variable's incoming value while
the process still performs an observable write to that variable.

For example, a final procedural assignment of a constant may overwrite an
external write whenever the process executes. Replacing it with
initialization would not preserve that behavior. Splitting a process into
separate assignments can also change which input changes cause public
storage to be rewritten. Retained bits and array elements must continue to
reflect external writes.

These requirements are one reason DFG synthesis conservatively rejects
observed intermediate values of externally writable variables. Validating
such transformations requires checking public storage after evaluation, as
well as the process outputs.
