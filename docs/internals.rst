...
Each parameterized reference has one binding fact per formal parameter.
Once the bindings and any nested specialization environments are available,
``V3Elab`` installs the complete declaration-order binding tuple.
``V3Param`` uses that tuple when cloning or reusing module, interface, and
class specializations, so specialization identity does not depend on
traversal order or on whether an actual was written explicitly or supplied
by a default.

Class specializations produced by ``V3Param`` publish the type and member
projections consumed by dependent expressions. AST replacements occur only
at convergence boundaries. The graph is destroyed before parameterization
subtrees retained for graph identity are deleted. After parameterization
and name relinking, the same dependency model resolves deferred parameter
values.
